...
include_labels = ["resistor", "capacitor_polarized", "capacitor_unpolarized", "diode", "voltage_dc", "voltage_dc_ac", "inductor"]

image_files = [t for t in image_files if Path(t).parent.stem in include_labels]

image_files_sub = random.sample(image_files, 7000)
...
# pipe = Pipeline([('model', RandomForestClassifier(verbose=11, n_estimators=100))])
pipe = Pipeline([('model', MLPClassifier((128,128,64,64,32), activation="relu", verbose=11))])
